...
class ImagePack
{
    ...
    static boolean compare(ArrayList<int[]> al, ArrayList<int[]> bl) {
        ...
        for (int i=0; i<al.size(); i++) {
            ...
            c += Math.abs(b[0] - a[0]) + Math.abs(b[1] - a[1]) + Math.abs(b[2] - a[2]);
        }
    ...
    static ArrayList<int[]> getPixels(BufferedImage bi) {

        ArrayList<int[]> pixels = new ArrayList<>();

        for (int y = 0; y < bi.getHeight(); y++) {
            for (int x = 0; x < bi.getWidth(); x++) {
                pixels.add(bi.getRaster().getPixel(x, y, new int[3]));
            }
        }
    ...
    static BufferedImage getScreenshot(Rectangle bounds)
    {
